...
async fn disallow_non_owner() {
    ...
    let read = ReadBuilder::new()
        ...
        .await
        .expect("should create read");
    let Err(Error::Forbidden(e)) = node.request(read).owner(alice.did()).execute().await else {
...
async fn published_authenticated() {
    ...
    let read = ReadBuilder::new()
        ...
        .expect("should create read");
    let reply =
        node.request(read.clone()).owner(alice.did()).execute().await.expect("should write");
    assert_eq!(reply.status, StatusCode::OK);
...
async fn deleted_write() {
    ...
    MessageStore::put(&node.provider, alice.did(), &delete).await.expect("should save");
    ...
    let read = ReadBuilder::new()
        ...
        .await
        .expect("should create read");
    let Err(Error::BadRequest(e)) = node.request(read).owner(alice.did()).execute().await else {
...
async fn root_role() {
    ...
    let read = ReadBuilder::new()
        ...
        .expect("should create read");
    let reply =
        node.request(read).owner(alice.did()).execute().await.expect("should configure protocol");
    assert_eq!(reply.status, StatusCode::OK);
}
...
async fn context_role() {
    ...
    let participant = WriteBuilder::new()
        ...
        .expect("should create write");
    let reply =
        node.request(participant.clone()).owner(alice.did()).execute().await.expect("should write");
    assert_eq!(reply.status, StatusCode::ACCEPTED);
...
async fn invalid_context_role() {
    ...
    let thread_1 = WriteBuilder::new()
        ...
        .expect("should create write");
    let reply =
        node.request(thread_1.clone()).owner(alice.did()).execute().await.expect("should write");
    assert_eq!(reply.status, StatusCode::ACCEPTED);
    ...
    let thread_2 = WriteBuilder::new()
        ...
        .expect("should create write");
    let reply =
        node.request(thread_2.clone()).owner(alice.did()).execute().await.expect("should write");
    assert_eq!(reply.status, StatusCode::ACCEPTED);
...
async fn data_blocks_deleted() {
    ...
    DataStore::delete(&node.provider, alice.did(), &write.record_id, &write.descriptor.data_cid)
        .await
...
async fn decrypt_context() {
    ...
    let reply = node.request(query).owner(alice.did()).execute().await.expect("should match");
    assert_eq!(reply.status, StatusCode::OK);
    ...
    // --------------------------------------------------
    let reply = node.request(write.clone()).owner(bob.did()).execute().await.expect("should write");
    assert_eq!(reply.status, StatusCode::ACCEPTED);
    ...
    let reply = node.request(read.clone()).owner(bob.did()).execute().await.expect("should write");
    assert_eq!(reply.status, StatusCode::OK);
...
async fn decrypt_protocol() {
    ...
    let email = ConfigureBuilder::new()
        ...
        .expect("should build");
    let reply =
        node.request(email).owner(alice.did()).execute().await.expect("should configure protocol");
    assert_eq!(reply.status, StatusCode::ACCEPTED);
...
async fn invalid_signature() {
    ...
    let Err(Error::Unauthorized(e)) = node.request(read).owner(alice.did()).execute().await else {
