...
async fn no_grant() {
    ...
    let query = QueryBuilder::new().sign(&alice_signer).build().await.expect("should create write");
...
async fn invalid_request() {
    ...
    let mut query =
        QueryBuilder::new().sign(&alice_signer).build().await.expect("should create query");
