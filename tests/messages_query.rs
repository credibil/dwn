...
async fn owner_messages() {
    ...
    let reply =
        alice_node.request(message).owner(alice.did()).execute().await.expect("should write");
    assert_eq!(reply.status, StatusCode::ACCEPTED);
...
async fn no_grant() {
    ...
    let Err(Error::Forbidden(e)) = bob_node.request(query).owner(bob.did()).execute().await else {
